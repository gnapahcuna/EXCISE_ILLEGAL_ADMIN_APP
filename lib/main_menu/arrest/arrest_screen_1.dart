...
class _ArrestMainScreenFragmentState extends State<ArrestMainScreenFragment>  with TickerProviderStateMixin {
      ...
      if(onValue!=null){
        _itemsInitTab6.forEach((item){
          ...
          ArrestIndictment.add({
            ...
            "ArrestIndictmentProduct": ArrestLawbreaker.isNotEmpty
                ?[]
                :ArrestProductInside
          });
        });
  ...
  bool IsUpProduct) async {
    ...
    if (!IsUpAddIndicmen) {
      ...
      for(int i=0;i<list_add_indicment.length;i++){
        for(int j=0;j<list_add_indicment[i].ArrestIndictmentProduct.length;j++){
          if(item_indicment.ArrestIndictment[i].ArrestIndictmentDetail.length>0){
            for(int k=0;k<item_indicment.ArrestIndictment[i].ArrestIndictmentDetail.length;k++){
              ArrestProduct.add({
                "PRODUCT_INDICTMENT_ID": "",
    ...
    if (!IsUpIndicmenDelt) {
      List<ItemsArrestIndictmentDetail> item_indic_detail = [];
      await future.apiRequestArrestIndictmentDetailinsAll(map_up_indic_delt)
          .then((onValue) {
        if (onValue.IsSuccess.endsWith("True") &&
            onValue.Msg.endsWith("Complete")) {
          print("Up Add Indictment Detail : " + onValue.Msg);
          item_indic_detail = onValue.ArrestIndictmentDetail;
          //Navigator.pop(context);
        }
      });
      print(item_indic_detail.length);
      List<Map> ArrestProduct=[];
      item_indic_detail.forEach((item){
        for(int i=0;i<list_update_indicment.length;i++){
          for(int j=0;j<list_update_indicment[i].ArrestIndictmentProduct.length;j++){
            ArrestProduct.add({
              "PRODUCT_INDICTMENT_ID": "",
              "PRODUCT_ID": _arrestMain.ArrestProduct[list_update_indicment[i].ArrestIndictmentProduct[j].INDEX].PRODUCT_ID,
              "INDICTMENT_ID": item.INDICTMENT_DETAIL_ID,
              "SIZES_UNIT_ID": list_update_indicment[i].ArrestIndictmentProduct[j].SIZES_UNIT_ID,
              "QUATITY_UNIT_ID": list_update_indicment[i].ArrestIndictmentProduct[j].QUATITY_UNIT_ID,
              "VOLUMN_UNIT_ID": list_update_indicment[i].ArrestIndictmentProduct[j].VOLUMN_UNIT_ID,
              "SIZES": list_update_indicment[i].ArrestIndictmentProduct[j].SIZES,
              "SIZES_UNIT": list_update_indicment[i].ArrestIndictmentProduct[j].SIZES_UNIT,
              "QUANTITY": list_update_indicment[i].ArrestIndictmentProduct[j].QUANTITY,
              "QUANTITY_UNIT": list_update_indicment[i].ArrestIndictmentProduct[j].QUANTITY_UNIT,
              "VOLUMN": list_update_indicment[i].ArrestIndictmentProduct[j].VOLUMN,
              "VOLUMN_UNIT": list_update_indicment[i].ArrestIndictmentProduct[j].VOLUMN_UNIT,
              "FINE_ESTIMATE": "",
              "IS_ILLEGAL": 1,
              "IS_ACTIVE": 1
            });
      ...
      if(!ArrestProduct.isEmpty){
        await future.apiRequestArrestIndictmentProductinsAll(ArrestProduct)
            .then((onValue) {
          if (onValue != null) {
            print("Up Add Indictment Product : " + onValue.Msg);
          }
        });
      }
    }
  ...
  void initState() {
    ...
    if (widget.IsPreview) {
      ...
      _arrestMain.ArrestLocale.forEach((item){
        ...
        address = item.ADDRESS_NO+(item.ALLEY==null?"":((item.ALLEY.isEmpty)?"":" ซอย "+item.ALLEY))
            +(item.ROAD==null?"":((item.ROAD.isEmpty)?"":" ถนน "+item.ROAD))
      ...
      if (_onEdited) {
        ...
        if (list_delete_lawbreaker.length > 0 ||
          ...
          list_update_indicment.forEach((f) {
            f.ArrestIndictmentProduct.forEach((item) {
              /*if(item.PRODUCT_INDICTMENT_ID!=null){
          ...
          if(list_update_indicment.length>0){
            ...
            list_delete_indicment_pro.forEach((f){
              map_del_indic_product_id.add({
                "PRODUCT_INDICTMENT_ID": f
              });
            });
          }
        }
    ...
    if (result.toString() != "back") {
      setState(() {
        ...
        if(_onEdited){
          ...
          _arrestMain.ArrestIndictment[index].ArrestIndictmentProduct.forEach((item) {
            print(item.PRODUCT_INDICTMENT_ID.toString());
            if(item.PRODUCT_INDICTMENT_ID!=null) {
              list_delete_indicment_pro.add(item.PRODUCT_INDICTMENT_ID);
            }
          });
          var item = result;
                            ...
                            onTap: () {
                              ...
                              _itemsInitTab6[index].ArrestIndictmentProduct
                                ...
                                for (int i = 0; i <
                                    _itemsInitTab6[index].ArrestIndictmentProduct
                                        .length; i++) {
                                  if (f.PRODUCT_ID == _itemsInitTab6[index]
                                      .ArrestIndictmentProduct[i]
                                      .PRODUCT_ID) {
                                    _itemsInitTab6[index]
                                        .ArrestIndictmentProduct[i]
                                        .IsCheckOffence = true;
                    ...
                    Center(
                        child: InkWell(
                          onTap: () {
                            ...
                            //set checked ของกลาง
                            _arrestMain.ArrestIndictment[index]
                                .ArrestIndictmentProduct.forEach((f) {
                              for (int i = 0; i <
                                  _arrestMain.ArrestProduct.length; i++) {
                                if (f.PRODUCT_ID ==
                                    _arrestMain.ArrestProduct[i]
                                        .PRODUCT_ID) {
                                  _arrestMain.ArrestProduct[i]
                                      .IsCheckOffence = true;
                                  break;
